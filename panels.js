...
function classLabelFull(cl){
  const lbl = CLASS_LABEL[cl];
  if(lbl===undefined) return '';
  return /^[0-9]+$/.test(String(cl)) ? lbl+'-ാം ക്ലാസ്' : lbl;
}
...
function SadharPanel(){
  ...
  function sAttendance(){
    ...
    c.innerHTML = `<div class="font-display text-xl text-deep mb-4">✅ ഹാജർ (നേരിട്ട്)</div>
      <div class="grid grid-cols-2 gap-2 mb-4">
        <select id="clsSel" class="border-2 border-deep/15 rounded-lg px-3 py-2 text-sm">${classList.map(cl=>`<option value="${cl}">${classLabelFull(cl)}</option>`).join('')}</select>
  ...
  function sExam(){
    ...
    c.querySelector('#sendAlert').onclick = ()=>{
      ...
      const text = `📢 മമ്പഉൽ ഉലൂം മദ്റസ - ${classLabelFull(cls)} ${examLabel} പരീക്ഷ ടൈംടേബിൾ: ` + list.map(l=>`${l.subject} (${l.date} ${l.time})`).join(', ');
  ...
  function sStudents(){
    const c = el('div','px-5 py-5 max-w-md mx-auto');
    c.innerHTML = `<div class="flex items-center justify-between mb-4">
        <div class="font-display text-xl text-deep">📚 കുട്ടികളുടെ വിവരങ്ങൾ</div>
        <button id="addStuBtn" class="bg-deep text-white text-xs px-3 py-2 rounded-lg">+ പുതിയ കുട്ടി</button>
      </div>
      <select id="clsSel" class="w-full border-2 border-deep/15 rounded-lg px-3 py-2 mb-4">${classList.map(cl=>`<option value="${cl}">${classLabelFull(cl)}</option>`).join('')}</select>
    ...
    function loadList(){
      ...
      if(!students.length){ list.innerHTML = `<div class="text-center text-deep/40 text-sm py-6">ഈ ക്ലാസിൽ കുട്ടികളില്ല</div>`; return; }
      students.forEach(s=>{
        const row = el('div','arch-card bg-white border border-deep/10 p-3 flex items-center gap-3');
    ...
    c.querySelector('#addStuBtn').onclick = ()=>addStudent(c.querySelector('#clsSel').value);
    function addStudent(defaultCls){
      const m = Modal('പുതിയ കുട്ടിയെ ചേർക്കുക');
      m.body.innerHTML = `
        <input id="nReg" placeholder="രജിസ്റ്റർ നമ്പർ" class="w-full border-2 border-deep/15 rounded-lg px-3 py-2 mb-2"/>
        <input id="nName" placeholder="പേര്" class="w-full border-2 border-deep/15 rounded-lg px-3 py-2 mb-2"/>
        <input id="nFather" placeholder="രക്ഷിതാവിന്റെ പേര്" class="w-full border-2 border-deep/15 rounded-lg px-3 py-2 mb-2"/>
        <input id="nPhone" placeholder="ഫോൺ നമ്പർ" class="w-full border-2 border-deep/15 rounded-lg px-3 py-2 mb-2"/>
        <input id="nDob" type="date" placeholder="ജനനത്തീയതി" class="w-full border-2 border-deep/15 rounded-lg px-3 py-2 mb-2"/>
        <select id="nCls" class="w-full border-2 border-deep/15 rounded-lg px-3 py-2 mb-3">${classList.map(cl=>`<option value="${cl}" ${cl===defaultCls?'selected':''}>${classLabelFull(cl)}</option>`).join('')}</select>
        <button id="nSave" class="w-full bg-deep text-white rounded-lg py-2.5 font-medium">ചേർക്കുക</button>
      `;
      m.body.querySelector('#nSave').onclick = async ()=>{
        const reg = m.body.querySelector('#nReg').value.trim();
        const name = m.body.querySelector('#nName').value.trim();
        const father = m.body.querySelector('#nFather').value.trim();
        const phone = m.body.querySelector('#nPhone').value.trim();
        const dob = m.body.querySelector('#nDob').value;
        const cls = m.body.querySelector('#nCls').value;
        if(!reg || !name){ toast('രജിസ്റ്റർ നമ്പറും പേരും നൽകുക', false); return; }
        const existing = await db.ref('students/'+reg).get();
        if(existing.exists()){ toast('ഈ രജിസ്റ്റർ നമ്പർ നിലവിലുണ്ട്', false); return; }
        await db.ref('students/'+reg).set({name, father, phone, dob, cls, shift:String(CLASS_SHIFT[cls]||''), deleted:false});
        toast('കുട്ടിയെ ചേർത്തു'); m.close(); paint();
      };
    }
...
function TeacherPanel(){
  ...
  function tClasses(){
    ...
    myClasses.forEach(cls=>{
      const box = el('div','arch-card bg-white border border-deep/10 p-4 mb-3');
      box.innerHTML = `<div class="font-medium text-deep mb-2">${classLabelFull(cls)}</div>`;
  ...
  function tAttendance(){
    ...
    c.innerHTML = `<div class="font-display text-xl text-deep mb-4">✅ ഹാജർ രേഖപ്പെടുത്തൽ</div>
      <div class="grid grid-cols-2 gap-2 mb-4">
        <select id="clsSel" class="border-2 border-deep/15 rounded-lg px-3 py-2 text-sm">${myClasses.map(cl=>`<option value="${cl}">${classLabelFull(cl)}</option>`).join('')}</select>
        <input id="dateSel" type="date" value="${todayISO()}" class="border-2 border-deep/15 rounded-lg px-3 py-2 text-sm"/>
      </div>
  ...
  function mDefaulters(){
    ...
    function load(){
      ...
      defaulters.forEach(([reg,s])=>{
        ...
        row.innerHTML = `<div class="flex-1"><div class="font-medium text-sm">${s.name}</div><div class="text-[11px] text-deep/40">${classLabelFull(s.cls)} · Reg ${reg}</div></div>
  ...
  function mFeeReport(){
    ...
    function load(){
      ...
      classList.forEach(cls=>{
        ...
        row.innerHTML = `<div class="text-sm font-medium">${classLabelFull(cls)}</div><div class="text-sm"><span class="text-deep2 font-medium">${paid}</span> / ${stu.length} അടച്ചു</div>`;
        list.appendChild(row);
      });
    }
    c.querySelector('#moSel').onchange = load; setTimeout(load,0);
  ...
  function pInfo(){
    ...
    c.innerHTML = `<div class="font-display text-xl text-deep mb-4">📋 കുട്ടിയുടെ വിവരങ്ങൾ</div>
      <div class="arch-card bg-white border border-deep/10 p-4 mb-4 flex items-center gap-3">
        ...
        <div><div class="font-medium">${child?.name||''}</div><div class="text-xs text-deep/50">${classLabelFull(child?.cls)} · ഷിഫ്റ്റ് ${CLASS_SHIFT[child?.cls]}</div><div class="text-xs text-deep/40">Reg: ${state.user.reg}</div></div>
      </div>
